...
Two Claude Code hooks activate automatically when `MEMPAL_ENABLED` is set:
...
When `MEMPAL_ENABLED` is unset the hooks are registered but short-circuit immediately, so there is no overhead.

| Variable | Default | Purpose |
|---|---|---|
| `MEMPAL_ENABLED` | `false` | Set to `true` to activate MemPalace hooks |
